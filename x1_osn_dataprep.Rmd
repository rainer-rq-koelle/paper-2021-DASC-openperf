...
calc_pairwise_spacing <- function(.uid, .seq_uid, .ds, .bins = bins){
...
rqq <- spd %>% 
  ...
  roll_median(MIN_DIST_OFF, 15, DIST_SPACING) %>%
  roll_median(MIN_TIME_OFF, 15, TIME_SPACING) %>%
  roll_median(DIST_OFF, 15, DIST_LF) %>%
...
```{r}
#tmp <- rqq %>% filter(GID <= 20, STEP %in% seq(from = 1, to = 3600, by = 5))
tmp <- rqq %>% filter(GID <= 20) %>% mutate(STEP = STEP %/% 5)
...
```{r}
tmp %>% filter(STEP >= 300, 600 <= STEP) %>%
  ggplot() +
    geom_boxplot(aes(x = STEP, y = DIST_SPACING, group = STEP))
```

```{r}
tmp %>% filter(STEP >= 600, 900 <= STEP ) %>%
  ggplot() +
    geom_boxplot(aes(x = STEP, y = DIST_SPACING, group = STEP))
``` 
